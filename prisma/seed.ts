...
// Hashes refer to the files in the `uploads/` directory
const uploads = [
    // Bylaws
    {
        documentId: 1,
...
] as const satisfies UploadCreateManyInput[]
...
] as const satisfies MeetingMinutesCreateManyInput[]

function uploadExists(hash: string): boolean {
    return existsSync(path.resolve(process.cwd(), 'uploads', hash))
}

await prisma.upload.createMany({
    data: uploads.filter(upload => uploadExists(upload.hash)),
})

await prisma.meetingMinutes.createMany({
    data: meetingMinutes.filter(minutes => uploadExists(minutes.hash)),
})
